...
function startBDS(onstarted) {
  const CJSModule = NativeModule.require('internal/modules/cjs/loader');
  NativeModule._nodegate(onstarted, CJSModule._load,
                         console.log.bind(console),
                         console.error.bind(console),
                         process._tickCallback);
  process._tickCallback();
  console.log('[BDSX] Server stopped');
  setTimeout(() => {
...
function executeUserCode() {
  ...
  process.stdout.write = function() {
    process_stdout_write.apply(this, arguments);
    NativeModule._nodegate_stdout.apply(NativeModule, arguments);
  };

  const process_stderr_write = process.stderr.write;
  process.stderr.write = function() {
    process_stderr_write.apply(this, arguments);
    NativeModule._nodegate_stderr.apply(NativeModule, arguments);
  };
  ...
  if (process._forceRepl || NativeModule.require('tty').isatty(0)) {
    ...
    if (process._eval != null) {
      ...
      startBDS(() => {
        console.log('[BDSX] Entry point is not defined');
      });
    }
    return;
  }
